...
// Dada una posición(fil,col), y un jugador, devuelve el número de 2 en línea que tiene ese jugador
...
double Valoracion(const Environment &estado, int jugador){
	...
	for (int i=0; i<7; i++){
		for (int j=0; j<7; j++){
			...
			num3Linea -= EnLinea3(estado,i,j,jugador);
			num4Linea -= EnLinea4(estado,i,j,jugador);
		}
	}
		...
		if (jugador == MAX){						// Si nos encontramos en un nodo MAX, los hijos serán MIN
			...
			for (int i=0; i<8; i++){	// Para cada posible sucesor, generamos su nodo en el árbol
				...
				if (posibles[i]==true and j<n_act){	// Si se trata de un movimiento posible, generamos y evaluamos el sucesor
					/*if (prof == 0){
						cout << "GENERANDO HIJO de accion " << i << endl;
					}*/
					...
					if (average < valorHijo){	// Al estar en un nodo MAX, su evaluación es el máximo de las evals de los hijos
						...
						/*if (prof ==0){
							cout << "MAX--CAMBIO EN LA ACCION A " << i << endl;
						}*/
