...
double Valoracion(const Environment &estado, int jugador){
	...
	double valor = 0;
	int num3Linea = 0;		// 3 en linea nuestras (QUEREMOS MINIMIZAR)
	int conex = 0;
	...
	for (int i=0; i<7; i++){
		for (int j=0; j<7; j++){
			...
			conex = EnLinea3(estado,i,j,jugador);
			num3Linea -= conex;
...
Environment::ActionType Player::Think(){
	...
	//int conex = EnLinea(actual_,6, 4, jugador_);
	//cout << "POS (6,4) CON CONEXIONES " << conex << endl;
